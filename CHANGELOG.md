...
### Changed
- **The two "where is your project?" errors now tell an agent what to do next.** "No CodeGraph project is loaded" explained how to pass `projectPath` without saying which paths exist; it now lists the indexed projects visible from the directory it searched (a bounded look — three levels deep, ten results, a few hundred milliseconds, never descending into a project or `node_modules`). "CodeGraph not initialized in X" said to run `codegraph init` in that project first; it now spells out the command with the path, says indexing only reads your code and takes about two seconds for a 200-file project, and that the running server picks the new index up by itself. Between them these two errors were eight of twenty-one failed calls in one local transcript corpus, and every one of them sent the agent back to the shell.
...
### Fixed
- **`npm test` no longer writes into the developer's own git hooks directory.** `__tests__/git-hooks.test.ts` created a throwaway repo and let it inherit a global `core.hooksPath`, so the hook installer resolved to the real shared hooks directory instead of the temp repo — five failing tests on any machine that sets one globally, and a rewritten hook file on machines where the write succeeds. The temp repo now pins `core.hooksPath` locally.
